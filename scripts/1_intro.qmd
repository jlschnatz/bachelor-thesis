...
The present study introduces $\mathrm{SPEEC}$ ($\mathbfsfup{S}\text{imulation-based}~\mathbfsfup{P}\text{ublication bias}~\mathbfsfup{E}\text{stimation and} ~ \mathbfsfup{E}\text{ffect size} ~ \mathbfsfup{C} \text{orrection}$), a novel simulation-based framework to assess the extent of publication bias in meta-analyses and estimate and correct effect sizes under the presence of publication bias. 
...
The central idea of the SPEEC method is to explicitely define a generative model of publication bias that incorporates assumptions about the marginal distributions of effect size (Gaussian distribution) and sample size (Negative-Binomial) as well the impact of publication bias.
...
Distributional parameters that control

From this generative model, simulation of theoretical data 
