...
Comparison between correlation between effect size and sample size as an indicator of publication bias and the newly proposed publication bias parameter $\pbs$ from the SPEEC approach ... it still can be expected that estimated publication bias parameter $\epbs$ is positively associated with the Fisher *z*-transformed Spearman correlation coefficients of the association between unsigned effect size and sample size in each meta-analysis. In other words, when the proposed method estimates high publication bias (i.e., low probabilities for $\epbs$ ) we expect the correlation coefficients for each meta-analysis to be more negative and conversely. In statistical terms, this expectation implies that the regression coefficient $\beta_{z}$ will be greater than zero.

$$
\begin{gathered}
\mathcal{H}_0: \beta_{z_{r_s}} \leq 0 \\
\mathcal{H}_1: \beta_{z_{r_s}} > 0
\end{gathered}
$$ {eq-h1}

**Hypothesis II**: 

In instances where the true effect size of a research phenomenon is exactly zero ($\delta=0$) and substantial publication bias is evident, the distribution of effect size and sample size should display a symmetric hollowing around zero [@light_summing_1984]. This is because only studies with either large positive effect or large negative effects will be statistically significant and thus more likely to be published. Thus, due to this symmetry, the average effect size $\widehat{\delta}$ will not be biased, because theoretically negative and positive effect should roughly "outweigh" each other. This means that the difference $\Delta_{\mu_d}$ between the average effect size $\widehat{\delta}$ the estimated mean parameter $\mu_d$ of the effect size distribtion from the SPEEC approach should remain invariant irrespective of the magnitude of publication bias. However, when the true effect size exceeds zero ($\delta>0$), publication bias leads to an overestimation of the true effect (i.e. $\widehat{\delta}>\delta$), and conversely, overestimation in the opposite direction (i.e., $\widehat{\delta} < \delta$) when $\delta<0$. If the estimated mean parameter $\widehat{\mu}_d$ from the SPEEC appraoch is a better estimate of the true effect size $\delta$ under publication bias than the simple average effect size $\widehat{\delta}$, then the relationship between the difference $\Delta_{\mu_d}$ of those parameters and the publication bias parameter $\pbs$ should follow a curvilinear inverse u-shaped pattern. In other words, when the observed difference $\Delta_{\widehat{\mu}_d}$ is approaching zero, publication bias severity is expected to decrease (indicated by larger values for $\widehat{\omega}_{\text{PBS}}$). Conversely, when the difference $\Delta_{\widehat{\mu}_d}$ diverges from zero in both negative and positive directions, publication bias severity is expected to increase (i.e. lower values for $\widehat{\omega}_{\text{PBS}}$). In statistical terms, it is anticipated that the quadratic term $\beta_{\Delta_{\mu_d}}$ of the regression model is smaller than zero.

$$
\begin{gathered}
\mathcal{H}_0: \quad \beta_{\Delta_{\mu_d}} \geq 0 \\ 
\mathcal{H}_1: \quad \beta_{\Delta_{\mu_d}} < 0
\end{gathered}
$$ {eq-h2}
...
Moreover, when comparing meta-analysis which were published within a tradtional publishing system compared to multisite replication studies and registered replication reports, one can also make predictions about the publication bias parameter $\pbs$ Expected that the publication bias parameter $\pbs$ is greater for multisite replication studies in comparison to traditional meta-analysis. Relative comparison between traditional meta-analyses and multisite replication studies. Which means that relative to traditional meta-analysis, non-signicant studies within registered replication reports are more likely to be published in comparison. Not an absolute prediction, because in theory the $\pbs$ should equal one for multisite replication studies, as non-significant studies should not be less likely to be published within this publishing framework in comparison to significant replications. Rather, the pool of to be conducted studies within these large-scale replication projects is predetermined, and are published regardless of the outcome. However, this point prediction would be untestable in this context. In statistical terms, when the regressor is a binary indicator of the type of research synthesis with the reference level being the publication biased absent multisite replication studies ($MR$) and the outcome is the estimated publication bias parameter $\pbs$, the regression coefficient $\beta_{MR}$ can be expected to be greater than zero.
