...
#Delta <- 0
#(coef(mod_h2)[2] * (2*Delta + 1))
#(diff(predict(mod_h2, newdata = data.frame(Delta = c(Delta, Delta + 1)), type = "link")))

r2_ferrari_cribari <- format_value(cor(qlogis(mod_h2$model$w_pbs), predict(mod_h2, type = "link"))^2, digits = 3)

```

Concerning $\mathcal{H}_2$, panel B of figure 1 depicts the relationship between estimated publication bias parameter as a function of the dfference between the average effect size $\widehat{\delta}$ and the estimated mean parameter of the Gaussian effect size distribution $\widehat{\mu}_d$. The corresponding estimated quadratic slope negative and statistically significant `r string_h2`. The standardized odds ratio of the quadratic slope was $OR$ = `r f_std_or` $SD(\Delta_{\widehat{\mu}_d,\widehat{\delta}})=$ `r f_sd_delta`. This means, that one standard deviation change in the predictor variable $\Delta_{\widehat{\mu}_d,\widehat{\delta}}$ leads to `r f_std_or` times reduction in $\omega_{\text{PBS}}$. The overall explained variation of $\omega_{\text{PBS}}$ by $\Delta_{\widehat{\mu}_d,\widehat{\delta}}$ as determined by the pseudo $R^2$ @ferrari_beta_2004 was low $R^2_{\text{pseudo}}=$ `r r2_ferrari_cribari`.
