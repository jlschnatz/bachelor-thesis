...
```
As this study relies on empirical data to preliminarily assess the proposed SPEEC approach, the true values for the distributional parameters and the publication bias parameter remain unknown. However, as already discussed, because publication bias is inherently absent by design in multisite replication studies and registered reports. Thus, the four distributional parameters ($\mu_d$, $\sigma^2_d$, $\mu_n$, $\phi_n$) within the SPEEC approach cannot be biased due to publication bias (especially the mean and variance of the effect size distribution). Because of this fact, we can use the subset of the data containing the multisite replication studies and registered reports to diagnostically evaluate the parameter estimation of SPEEC. We can compare the distributional parameters estimated via SPEEC to the distributional parameters estimates via Maximum Likelihood (MLE) -> they should be (roughly) equivalent.
...
5.  Is the discrepancy between SPEEC and MLE in the distributional parameters associated with the publication bias parameter $\omega_{\text{PBS}}$?


We used the BFGS optimization algorithm the obtain the ML estimates for the mean $\mu_nd$ and variance $\sigma^2_d$ of the Gaussian effect size distribution and mean $\mu_n$ and dispersion $\phi_n$ of the Negative-Binomial sample size distribution. Additionally, we calculated the mean and median discrepancy between SPEEC and MLE descriptively to assess the average difference between the two estimation methods.

Figure 3 provides a visual summary of the analysis comparing the distributional parameters estimated by the SPEEC method against those estimated by Maximum Likelihood Estimation (MLE). The diagonal line signifies perfect alignment between MLE and SPEEC estimates. Values below the diagonal indicate higher values for MLE compared to SPEEC, while values above the diagonal indicate the opposite.

Panel A1 reiterates the findings of the analysis of $\mathcal{H}_3$, suggesting a small discrepancy between the two methods in estimating the mean of the Gaussian effect size distribution (`r str_discr(discr_ml_speec, "delta_mu_d")`). This discrepancy is deemed practically negligible according to the equivalence test. However, the remaining panels indicate contrasting outcomes.

Panel A2 illustrates a systematic discrepancy in estimating the variance parameter of the Gaussian effect distribution between SPEEC and MLE (`r str_discr(discr_ml_speec, "delta_sigma2_d")`).

This discrepancy is evident as most points are positioned above the diagonal. Notably, the overestimation in SPEEC increases exponentially with rising variance estimates from MLE, displaying substantial heteroscedasticity.

from the two approaches (`r str_discr(discr_ml_speec, "delta_sigma2_d")`). This is indicates as most points are located in above the diagonal. However, not only overestimation, variance estimated by MLE increases, the overestimation in SPEEC increases following an exponetial looking trend and displaying substantial heterscedasticity (increasing variance as the estimate of ML for parameter increases).

Similarly, Panel B1 also illustrates a systematic overestimation of the mean parameter of the Negative-Binomial sample size distribution by SPEEC in comparison to MLE (`r str_discr(discr_ml_speec, "delta_mu_n")`), which intensifies with higher mean parameter estimates from MLE.

Panel B2 shows that the SPEEC approach generally understimates dispersion parameter of the sample size distribution in comparison to the ML estimate (`r str_discr(discr_ml_speec, "delta_phi_n")`) and also furthermore indicates a systematic relationship in the misestimation.
...
Pairwise correlational analysis of the absolute absolute difference in the parameter estimates from the two estimation methods together with additional variables: publication bias parameter $\widehat{\omega}_{\text{PBS}}$ and the total number of primary replication studies $k$ within each multisite replication project or registered report.

Table XX summarises the results of the correlational analysis.

Strong positive correlation between absolute difference between ML and SPEEC estimation of mean parameter and variance parameter of the Gaussian effect size distribution.This means that as the discrepancy between SPEEC and ML increases for mean parameter $\mu_d$, the discprencany also increases for the the variance parameter $\sigma^2_d$ of the effect size distribution.

Moreover strong negative correlations between publication bias parameter $\omega_{\text{PBS}}$ and divergence of between ML and SPEEC of the mean and variance parameter of the effect size distribution.

As the discrepancy between ML and SPEEC estimation increases for both mean $\lvert\Delta_{\mu_d}\rvert$ and variance parameter $\lvert\Delta_{\sigma^2_d}\rvert$, the publication bias parameter $\omega_{\text{PBS}}$ decreases, indicating more severe predicted publication bias.

Notably, no distributional parameter (divergence between ML and SPEEC) was significantly correlated with the total number of primary replications $k$.
