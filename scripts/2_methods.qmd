...
Given the definitions of the marginal distributions, $k$ individual studies are sampled from the joint distribution of effect size and sample size. The selection of $k$ is user-defined, but generally the larger the number of random samples $k$, the lower the uncertainty of the joint effect size sample size distribution will be. In general, there is a tradeoff between increased computational resources and the reduced uncertainty of the joint distribution for higher values of $k$. We opted for $k=10^4$ random samples for each simulation iteration.

### Application of Publication Bias

Following the simulation step of sampling $k$ individual studies from the joint distribution of effect size and sample size conditional on the distributional parameters, the subsequent step entails the application of publication bias to these random samples. As mentioned in the introduction, we operationalize publication bias in terms of the likelihood of a study being published conditional on the statistical significance of its results. Translated to this simulation setting we can calculate the two-tailed *p*-value of each individual study $i$ from the random samples of effect size $d_i$ and sample size $n_i$. We presume that individual studies $i$ originate from a balanced sample size design, where the group sample sizes $n_{1i}$ and $n_{2i}$ are defined as $n_i/2$ when the total sample size is even. If the total sample size is odd, the group sample sizes are determined as the ceilinged $\lceil n_i/2 \rceil$ and and floored $\lfloor n_i/2 \rfloor$ values, respectively. To calculate the *p*-value $p_i$ of each simulated study, derived from *t*-value $t_i$
...
where $\pbs \in \mathbb{R} : 0 \leq \pbs \leq 1$. This weight denotes the probability of a study $i$ being selected conditional on the *p*-value and the type I error rate $\alpha$. If a study $i$ was not statistically significant ($p_i \geq \alpha$), a publication bias weight $\pbs$ is assigned, else the probability of a study being selected is 1, indicating no publication bias. Thus, the publication bias parameter $\pbs$ denotes the relative probability of a nonsignificant study being selected relative relative to a significant study being selected. We assume a fixed type I error rate for all simulated studies at the commonly chosen significance threshold of $\alpha = .05$ for the analysis of the hypotheses. However, user may define a different $\alpha$-level depending on their specific needs.

Following the computation of publication bias weight $\omega_{\text{PBS}_i}$ for each study $i$, the likelihood of a study being selected can be expressed as $\mathbb{P}(S_i = 1)= \omega_{\text{PBS}_i}$. Here, $S_i$ is a binary indicator function, signifying whether study $i$ is selected during the publication bias process.
