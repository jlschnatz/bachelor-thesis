...
Description of complete left side of the @fig-speec-workflow, describing the publication bias model with inputs: distributional parameters and publication bias parameter and output, scalar value of the Kullback-Leibler divergence between estimated joint kernel density of the simulated theoretical data from and the empirical meta-analytical data. This framework constitutes an optimization problem where, to find parameter values for such that the 
