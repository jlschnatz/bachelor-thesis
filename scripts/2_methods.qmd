...
The first step of the SPEEC method involves the definition of the marginal distributions of the effect size and sample size for the generative publication bias model. In defining these marginal distribution comes requires additional assumptions from which type of study design the effect sizes and samples sizes that are simulated were generated. For the purpose of this, because we work with Cohen´s *d* as a very popular effect size measure, we assume that the simulated effect sizes originate from a two-sample *t*-test design. However, the SPEEC framework itself could also apply if we would assume a different study design for which other effect size measures are typically reported (e.g., correlational effect sizes, effect sizes based on proportional data, ANOVA). This would require a potential adaption of the marginal distribution for the effect size and how the statistical significance of each simulated study is determined based on a difference effect size measure for the application of publication bias.
